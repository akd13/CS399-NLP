...
for dataset in datasets:
    if dataset != 'statista':
        desc_desc_jaccard = jaccard(statista['desc'], datasets_sets[dataset]['desc'])
        caption_caption_jaccard = jaccard(statista['cap'], datasets_sets[dataset]['cap'])
        context_context_jaccard = jaccard(statista['ctx'], datasets_sets[dataset]['ctx'])
        caption_context_jaccard = jaccard(statista['cap_ctx'], datasets_sets[dataset]['cap_ctx'])
        print(f"Jaccard similarity between statista and {dataset} description: {desc_desc_jaccard:.4f}")
        print(f"Jaccard similarity between statista and {dataset} caption: {caption_caption_jaccard:.4f}")
        print(f"Jaccard similarity between statista and {dataset} context: {context_context_jaccard:.4f}")
